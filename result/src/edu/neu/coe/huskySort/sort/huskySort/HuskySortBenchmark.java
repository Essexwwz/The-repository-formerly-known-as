...
public final class HuskySortBenchmark {
    ...
    void sortStrings(final Stream<Integer> wordCounts, final int totalOps) {
        System.out.println("sortStrings: beginning String sorts");
    ...
    private void doSortStrings(final int n, final int m) {
        ...
        if (isConfigBenchmarkStringSorter("english"))
            benchmarkStringSorters(COMMON_WORDS_CORPUS, HuskySortBenchmarkHelper.getWords(COMMON_WORDS_CORPUS, HuskySortBenchmark::lineAsList), n, m, HuskyCoderFactory.unicodeCoder);
    ...
    void sortLocalDateTimes(final int n, final int totalOps) {
        System.out.println("sortLocalDateTimes: beginning LocalDateTime sorts");
    ...
    void sortTuples(final int n, final int totalOps) {
        System.out.println("sortTuples: beginning Tuple sorts");
        final int m = round(totalOps / minComparisons(n));
    ...
    void sortNumerics(final int n, final int totalOps) {
        System.out.println("sortNumerics: beginning numeric sorts");
        final int m = round(totalOps / minComparisons(n));
    ...
    void benchmarkStringSorters(final String corpus, final String[] words, final int nWords, final int nRuns, final HuskyCoder<String> huskyCoder) {
        System.out.println("benchmarkStringSorters: testing pure sorts with " + formatWhole(nRuns) + " runs of sorting " + formatWhole(nWords) + " words using coder: " + huskyCoder.name());
    ...
    void benchmarkStringSortersInstrumented(final String[] words, final int nWords, final int nRuns, final HuskyCoder<String> huskyCoder) {
        System.out.println("benchmarkStringSortersInstrumented: testing with " + formatWhole(nRuns) + " runs of sorting " + formatWhole(nWords) + " words" + (config.isInstrumented() ? " and instrumented" : ""));
    ...
    public final static TimeLogger[] timeLoggersLinearithmic = {
            new TimeLogger("Raw time per run (mSec): ", (time, n) -> time+642*n/1000000),//Add a certain time to simulate the process of converting Chinese characters into pinyin
            new TimeLogger("Normalized time per run (n log n): ", (time, n) -> (time+642*n/1000000) / minComparisons(n) / 6 * 1e6)
    };
    ...
    private static void doPureBenchmark(final String[] words, final int nWords, final int nRuns, final Random random, final Benchmark<String[]> benchmark, final boolean preSorted) {
        ...
        System.out.println("CSV, " + benchmark + ", " + nWords + ", " + time);
    ...
    private static <Y extends Number & Comparable<Y>> void doNumericQuicksort(final String subject, final Supplier<Y[]> supplier, final int m, final Class<? extends Number> clazz, final boolean isInt) {
        if (clazz==Byte.class) {
            System.out.println("not attempting quicksort for: " + clazz);
            return;
        }
        if (isInt)
            try {
    ...
    private static void doQuicksort(final Number[] a, final boolean isInt, final Method method, final boolean isLong) {
        try {
            ...
            System.out.println("Exception performing quicksort on array of numeric objects");
        }
    }
    ...
    private static void logInterimInversions(final int nWords, final IntroHuskySort<String> sorter) {
        ...
        System.out.println("logInterimInversions: HuskySort interim inversions: " + asInt(mean));
        System.out.println("logInterimInversions: HuskySort first pass success rate: " + percentage + "%");
    }
    ...
    private final static TimeLogger[] timeLoggersQuadratic = {
            new TimeLogger("Raw time per run (mSec): ", (time, n) -> time+642*n/1000000),//Add a certain time to simulate the process of converting Chinese characters into pinyin
            new TimeLogger("Normalized time per run (n^2): ", (time, n) -> (time+642*n/1000000) / minComparisons(n) / 6 * 1e6)
    };

    static final String COMMON_WORDS_CORPUS = "javaio-appendfile.txt";

    static private void logBenchmarkRun(final double time) {
        System.out.println(TimeLogger.formatTime(time) + " ms");
    }
